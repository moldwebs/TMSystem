$(function() {
    $(document).on('click', '.saveRadioButton', function() {
        ...
        for (var i = 0; i < items.length; i++) {
            buttons += '<label class="btn ' + $(this).closest('.modal').find('.style').val() + ($(items[i]).find('.active').val() == 'active' ? ' active' : '') + '"><input type="radio" name="option" value="' + $(items[i]).find('.title').val() + '"' + ($(items[i]).find('.active').val() == 'active' ? ' checked' : '') + ' />' + $(items[i]).find('.title').val() + '</label>'
        }
        $(current_element).parent().find('.btn-group').html(buttons);
        $(this).closest('.modal').modal('hide')
    });
    $(document).on('change', '#bootstrapRadioButtons .active', function() {
        if ($(this).val() == 'active') {
...
function load_radioButtons(el) {
    ...
    $('#bootstrapRadioButtons').find('tbody').html('').append(structure);
    var style = size = '';
    ...
    $('#bootstrapRadioButtons').find('.style').val(style);
    ...
    $('#bootstrapRadioButtons').find('.size').val(size);
